	...
	try {
		...
		if (!isBotMentioned) {
			...
			if (rawText) {
				...
				const re = new RegExp(`@(?:${safeBot})(?:@[\\w.]+)?\\b`);
				heuristicMatched = re.test(rawText);
			}
		}

		if (!isBotMentioned && !heuristicMatched) return;
...
async function mentionToggleCommand(sock, chatId, message, args, isOwner, isMod) {
	if (!isOwner && !isMod) return sock.sendMessage(chatId, { text: 'Only Owner, Sudo, or Mods can use this command.' }, { quoted: message });
	const onoff = (Array.isArray(args) ? (args[0] || '') : args || '').trim().toLowerCase();
...
async function setMentionCommand(sock, chatId, message, isOwner, isMod) {
	if (!isOwner && !isMod) return sock.sendMessage(chatId, { text: 'Only Owner, Sudo, or Mods can use this command.' }, { quoted: message });
