...
const CONFIG_PATH = path.join(process.cwd(), 'data', 'antidelete.json'); // FIXED: absolute antidelete state path
...
From: @${senderName}`,
        ...
        if (text.includes('fetch failed') || text.includes('connection closed')) return;
...
module.exports = {
    ...
    async execute(ctx) {
        ...
        const match = rawText.replace(/^\.antidelete\b/i, '').trim().toLowerCase();
        return handleAntideleteCommand(ctx.sock || null, ctx.chatId || null, ctx.message || null, match);
    }
};
