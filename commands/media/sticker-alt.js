...
async function stickerCommand(sock, chatId, message) {
    try {
        ...
        const tempInput = `./tmp/temp_${Date.now()}.${type === 'imageMessage' ? 'jpg' : 'mp4'}`;
        const tempOutput = `./tmp/sticker_${Date.now()}.webp`;
...
module.exports = {
  name: 'sticker-alt',
  async execute(ctx) {
    return stickerCommand(ctx.sock || null, ctx.chatId || null, ctx.message || null);
  }
};
