...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        if (!q) {
            return await sock.sendMessage(chatId, { text: "Please provide text to generate\nExample: .metallic Nick" }, { quoted: message });
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await sock.sendMessage(chatId, { text: "Invalid text generator type" }, { quoted: message });
            }
...
module.exports = {
  name: 'textmaker',
  async execute(ctx) {
    return textmakerCommand(ctx.sock || null, ctx.chatId || null, ctx.message || null, ctx.q || null, ctx.type || null);
  }
};
